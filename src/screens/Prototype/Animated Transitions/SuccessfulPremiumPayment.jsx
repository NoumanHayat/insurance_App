...
import PaymentPlanScheduleModal from "../../../components/modals/renewals/PaymentPlanScheduleModal";
import moment from "moment";
import { PAUSE } from "redux-persist";

const SuccessfulPremiumPayment = ({ navigation, route }) => {
    ...
    const { paymentTerms, nextPayment, receipt_number, payment_number, payment_terms_number } = route.params;
    ...
    const youreCoveredStyles = useAnimatedStyle(() => {
        return {
            ...
            height: interpolate(youreCoveredSV.value, [0, 1], [0, 30 * ratioY]),
            transform: [{
                translateY: messageSV2.value,
            }]
        }
            ...
            <View style={styles.container}>
                <View style={styles.upperPlane}>
                    ...
                    <Animated.Text style={[styles.youreCovered, youreCoveredStyles]}>
                        {`Payment ${payment_number} of ${payment_terms_number} Made!`}
                    </Animated.Text>
                    <Animated.Text style={[styles.message, messageStyles]}>
                        Your policy is currently being updated.
                    </Animated.Text>
                    <Animated.Text style={[styles.message, messageStyles]}>
                        {payment_number < payment_terms_number ? 
                        <Animated.Text style={styles.boldDate}>Next payment is due on {moment(paymentTerms[payment_number].expiry_date).format("MMM DD, YYYY")}</Animated.Text>
                        :
                        ``
                        }
                    </Animated.Text>
                    <Animated.Text style={[styles.message, messageStyles]}>
                        If your policy isnt updated within 2 business days please contact AGIC with the following reference number.
                    </Animated.Text>
...
export default SuccessfulPremiumPayment;
