// From https://github.com/software-mansion/react-native-reanimated/issues/2826
// interpolateColor is an unstable function.

import {interpolate} from "react-native-reanimated";

function hexToRGBA(hw){
    'worklet';
    if (hw.length === 4){
        return {
...
export default function interpolateColorBugFix(value, inputRange, outputRange){
    'worklet';
    const outputRangeProcessed = outputRange.map(i => colorToRGBA(i));
    const values = {
        r: interpolate(
            value,
            inputRange,
